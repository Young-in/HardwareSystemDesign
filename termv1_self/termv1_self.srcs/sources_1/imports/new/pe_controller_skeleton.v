...
module pe_con#(
   ...
   // PE
    ...
    reg [31:0] wrdata_i [VECTOR_SIZE-1:0];
...
//            din <= rddata;
    ...
    wire [31:0] ld_val = (load_flag_en)? CNTLOAD1 :
                         ...
                         (harv_flag_en)? CNTHARV1 :
                         (done_flag_en)? CNTDONE :
                          'd0;
                         
    wire counter_ld = load_flag_en || calc_flag_en || harv_flag_en || done_flag_en;
    wire counter_en = load_flag || dvalid || harv_flag || done_flag;
    wire counter_reset = !aresetn || load_done || calc_done || harv_done || done_done;
    always @(posedge aclk)
        if (counter_reset)
            counter <= 'd0;
        else
    ...
    localparam CNTLOAD1 = (2*(VECTOR_SIZE + 1)*VECTOR_SIZE) -1;
    always @(posedge aclk)
        if (load_flag_reset)
            load_flag <= 'd0;
        else
    ...
    localparam CNTHARV1 = VECTOR_SIZE - 1;
    always @(posedge aclk)
        if(harv_flag_reset)
            harv_flag <= 'd0;
        else
    ...
    //S_LOAD: we
	assign we_local = (load_flag && counter[31:L_RAM_SIZE+1] && !counter[0]) ? 'd1 : 'd0;
	assign we_global = (load_flag && !counter[31:L_RAM_SIZE+1] && !counter[0]) ? 'd1 : 'd0;
	
	//S_CALC: wrdata
	genvar i;
	for(i = 0; i < VECTOR_SIZE; i = i + 1) begin: wrreg
...
//                    wrdata <= wrdata;
    ...
    assign rdaddr = (state == S_LOAD)? counter[2*L_RAM_SIZE+1:1] : 'd0;
